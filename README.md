...
用户进程拥有独立的地址空间，也就需要独立的页表（为了方便将页目录+页表统称页表），因此需要实现用户的页表映射。
...
值得注意的是，返回到应用层时，sysexit指令使用rdx和rcx的值赋值给rip和rsp，而rdx和rcx又被当作函数的参数，这里就产生了冲突，因此暂时把rip和rsp保存在r10和r11寄存器中。
在使用sysexit指令之前，需要将r10和rdx的值交换，将r11和rcx的值进行交换。

系统调用需要通过sysenter指令来使用，这样是不方便用户使用的，因此我们把sysenter指令给封装成宏，用户通过宏来使用系统调用会方便一些。
但是，通过宏来使用系统调用，用户需要知道系统调用功能号，用户需要传入系统调用功能好才能使用系统调用，仍然不方便。为了方便用户的使用，操作系统通常都会配套库函数，在库函数中进一步把宏封装成函数，于是用户直接使用库函数就能够触发系统调用了。这里一定要清楚的知道，库函数和用户程序一起编译的，因此库函数（封装好的系统调用）其实是位于内核空间的。

现在，我们并没有实现将用户程序从磁盘加载到内存中执行的功能，我们直接将内核空间中的函数复制到用户空间，这样其实也实现了用户进程。那我们现在是无法使用库函数的，于是我们通过宏来使用系统调用，这样也能够测试系统调用的正确性。
不过，我们仍然将封装了对应的系统调用函数，但是我们不进行编译。
